...
- あとはスライド読むだけ: 型みるだけで概要がわかるのはうれしい
-->
...
- **写像**と同じで、箱そのもの(リストの長さ、失敗しているかどうか)は変えない
...
fmap ((*2) . (+1)) [1, 2, 3] -- [4, 6, 8] 合成してから1回写像
...
<!--
- 圏論の関手則にしたがうようにfmapは実装される必要がある。
- つまり、箱から中身を取り出さずに、中身に手がだせる。マジシャンみたいだね
-->
...
- 非チェック例外にすれば`try-catch`は消えるが、**例外情報を型に載せられない**
...
<!--
- Eitherは失敗情報 or 成功した結果を持つ型
- findUserは失敗するかもというのが型でわかる
- nameとgreetはEither型のない普通の関数
- Functorの力で途中でtry-catch的なことをしなくてよくなっている
-->
...
### 例外情報を運ぶためだけのtry-catchが不要になりつつ、<br>型で失敗すること(例外)を表現できた!
