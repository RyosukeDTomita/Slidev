---
theme: ./theme
title: なぜHaskellは美しいと言われるのか?
info: "なぜHaskellは美しいと言われるのか? - Netadashi Meetup #17"
...
# なぜHaskellは美しいと言われるのか?

## 〜関数型言語と遅延評価を使ったプログラミングとは〜
...
<!--
今回の資料には多数ソースコードが含まれているため、スキャンしておくことを推奨します。
-->
...
<div class="tg-fib-cols grid grid-cols-2 gap-6 items-start">
<div>

**Python 3**: メモ化でキャッシュを持つ
...
- Haskellはシンプルで美しい記述ができる!
- 今日はその秘密を教えます
</Callout>

<style scoped>
/*
 * 2カラムに並べると既定のコードサイズでは横スクロールが出る。行数の雰囲気が
 * 伝わればよいスライドなので、ここだけコードを縮めて収める。
 * Slidevは`.slidev-code`のfont-sizeを!importantで上書きするため、
 * 変数(--slidev-code-font-size)側を差し替える。
 */
.tg-fib-cols {
  --slidev-code-font-size: 15px;
}

.tg-fib-cols pre.slidev-code {
  line-height: 1.5;
...
<!--
- 今日言いたいことは、Haskellは美しいコードがかけます
- なぜ美しいコードが書けるのかについて説明します
-->
...
<div class="tg-quote">
  <p class="tg-quote__body">誓約と覚悟が大きいほど念は強く働く!!</p>
  <p class="tg-quote__cite">クラピカ『HUNTER×HUNTER』(幻影旅団編)</p>
</div>
...
- Javaは`final`をつけて再代入を禁止できる
- Haskellでは変数への代入ではなく、値と名前を結びつける**束縛**(binding)
- 再代入はコンパイルエラー。更新するときは別の名前に束縛し直す
...
<!--
- Javaはfinalをつけて再代入を禁止できる。
- Haskellだと、変数という概念がないというのが近い。値と名前を結びつける束縛という言葉を使う。
- 再代入は不可で、更新したいときは別の名前に束縛し直す
-->
...
<!--
- 副作用なしでは現実世界に影響を及ぼすシステムが作れない。
- 副作用を起こす手続きを型付きの値にすることで関数を純粋に保ったまま副作用
- readFileはFilePathを受け取ってIO Stringを返す
- findは失敗を値として返す型Maybeをつけて返す
-->
...
<div class="tg-quote">
  <p class="tg-quote__body">プログラムは人が読むために書くものであり、機械が実行するのはついでである</p>
  <p class="tg-quote__cite">エイブルソン, サスマン『計算機プログラムの構造と解釈』(1984年)</p>
</div>
...
[Wikipedia](https://ja.wikipedia.org/wiki/%E9%96%A2%E6%95%B0%E5%9E%8B%E3%83%97%E3%83%AD%E3%82%B0%E3%83%A9%E3%83%9F%E3%83%B3%E3%82%B0)より

> 関数型プログラミングとは、数学的な意味での関数を主に使うプログラミングのスタイルである

Haskellは純粋関数の組み合わせでプログラムを記述する。

<!--
- 数学的な意味での関数=純粋関数を主に使うプログラミングスタイルを関数型という
- Haskellは純粋関数型
...
`total`の**書き換え**と**ループ**が、Haskellでは**制約**(純粋関数と束縛)により(基本的に)使えない
...
- Haskellのリストは1:2:3:[]のような構造になっており、演算子を:(コロン)を+に空リストを0にすると合計が求められる。
-->
...
<!--
- 畳み込みという同じ構造を使い、初期値と演算子を変えるだけでいろいろできる。
- 認知負荷が減って個人的にうれしい
-->

---

## 関数型は、定義を書くのでシンプルな記述に
...
<div class="tg-quote">
  <p class="tg-quote__body">人はいつか時間さえ支配することができるさ</p>
  <p class="tg-quote__cite">アムロ・レイ『機動戦士ガンダム 逆襲のシャア』(宇宙世紀0093年)</p>
</div>
...
```text
ghci> let x = 1 + 1  -- サンクが積まれる
ghci> print x        -- サンクが潰れて2になる
```
...
<!--
- 正格評価(普通の言語)では式は即座に評価されて値になる。1+1=2
...
- `cond = True`のとき`x`は2回現れるが、`expensive`の評価は1回だけ

(後ほど紹介するフィボナッチ数列の例で共有の真価が感じられる)

<!--
- JavaのStreamも実は遅延評価だが、Haskell特有の機能
...
- 補足: 今回は時間の都合上、遅延評価がうれしい例だけを紹介している
...
<div class="tg-quote">
  <p class="tg-quote__body">エレガント!! ベリーエレガント!!</p>
  <p class="tg-quote__cite">ヘンリー・ヘンダーソン『SPY×FAMILY』</p>
</div>
...
- Haskellは遅延評価を使うため、純粋関数・束縛・副作用も型に出すという制約がある
  - 関数型を使うのは制約のためであったが、結果として見通しの良いコードに
  - 遅延評価があることで表現力が拡張
- 遅延評価と関数型を組み合わせると相乗効果でより美しいコードが書ける
...
- `Functor`は「型の文脈(箱)の**中身**に関数を適用する」ための型クラス。圏論でいう関手の概念をHaskellに取り込んだもの
...
<span class="tg-focus">fmap ((*2) . (+1))</span> [1, 2, 3] <span class="tg-muted">-- [4, 6, 8]</span>
...
途中で失敗したかどうかの確認が必要なのは実装上の都合でしかない可能性がある。
-->
...
- Haskellは純粋関数型のため、数学(圏論)と親和性が高い。Functorもその一つ
- **処理が失敗したかどうかを中間層で確認せずに**上層まで処理結果を持ち上げることができる例を紹介
  - `fmap`でつなぐことで、型の文脈を壊さずに関数合成ができる
  - 例外ではなく、失敗情報 or 成功結果のどちらかを持つ`Either`を使って失敗情報を扱う
...
- Functor型クラスのfmapを使うことで文脈を壊さずに関数合成して処理が書ける
...
<style scoped>
/*
 * 参考文献は行数が多く、既定のサイズだと最後の1〜2件が画面外に落ちる。
 * 読み上げるものではなくQR越しに後から辿るページなので、ここだけ文字を詰める。
 */
.slidev-layout ul {
  font-size: 1.15rem;
  line-height: 1.35;
...
<span class="tg-muted">発表スライド「畳み込みで抽象度の高いコードに」の補足</span>
